...
use NaviteCore\LiquidOrm\QueryBuilder\Exception\QueryBuilderInvalidArguementException;
...
class QueryBuilder implements QueryBuilderInterface
{
    ...
    {
        if(count($args) < 0) {
            throw new QueryBuilderInvalidArguementException();
        }
    ...
    public function deleteQuery(): string
    {
        if($this->isQueryTypeValid("delete")) {
            ...
            $multiDelete = array_values($this->key['fields']);
            if(is_array($multiDelete) && count($multiDelete) > 1) {
    ...
    private function hasConditions()
    {
        ...
        if(isset($this->key['orderBy']) && $this->key['orderBy'] != '') {
            $this->sqlQuery .= " ORDER_BY " . $this->key['orderBy'] . " ";
        }
